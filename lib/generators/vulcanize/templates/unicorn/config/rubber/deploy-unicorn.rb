...
namespace :rubber do
  ...
  namespace :unicorn do
    ...
    task :reload, :roles => :unicorn do
      rsudo "if [ -f /var/run/unicorn.pid ]; then pid=`cat /var/run/unicorn.pid` && kill -HUP $pid; else cd #{current_path} && bundle exec unicorn_rails -c #{current_path}/config/unicorn.rb -E #{RUBBER_ENV} -D; fi"
    end
