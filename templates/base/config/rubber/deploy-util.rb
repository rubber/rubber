namespace :rubber do
  namespace :util do
    ...
    task :restore_cloud do
      ...
      (master_instances + slaves).each do |instance|
    ...
    task :local_to_cloud do
      ...
      task task_name, hosts: selected_db_instance.full_name do
        ...
        IO.read(File.join(File.dirname(__FILE__), '..', 'database.yml'))
        db = YAML.load(ERB.new(_).result)[Rubber.env]
        user = db['username']
        pass = db['password']
        pass = nil if pass && pass.strip.empty?
        host = db['host']
        name = db['database']

        unless rubber_env.db_backup_cmd
          raise 'No db_backup_cmd defined in rubber.yml, cannot backup!'
        end
        db_backup_cmd = rubber_env.db_backup_cmd.gsub(/%([^%]+)%/, '#{\1}')

        # use of `eval' is a serious security risk (Rubocop)
